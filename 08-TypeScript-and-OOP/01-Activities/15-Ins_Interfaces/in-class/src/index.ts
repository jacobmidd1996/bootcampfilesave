...
const serviceCar = (car: Vehicle) => {
  if (car.owner) {
    ...
    console.log(`It is owned by ${car.owner} whose name has ${car.owner.length} letters in it.`);
  } else {
...
function doCarThings(car: Vehicle): number {
  if (car.raceAtTopSpeed) {
    // Store the winner of a race between myCar2 and myCar3
    const winner = car.raceAtTopSpeed(myCar2, myCar3);
    // Log the winner
    console.log(
