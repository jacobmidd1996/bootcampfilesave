...
// This route will return a list of all terms available in the database
...
// This route will return the details of a specific term based on the provided term parameter in the URL.
...
app.get("/api/term/:term", (req: Request, res: Response) => {
  ...
  const term = termData.find(
    (term) => term.term.toLowerCase() === requestedTerm
  );
...
app.get("/api/terms/:category", (req: Request, res: Response) => {
  ...
  const result = termData.filter(
    (term) => term.category.toLowerCase() === requestedCategory
  );
