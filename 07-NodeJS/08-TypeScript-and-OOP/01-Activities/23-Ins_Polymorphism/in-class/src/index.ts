...
class Employee extends Person implements Reportable {
  ...
  override getFullName(): string { return `${super.getLastName()}, ${super.getFirstName()}, ${this.department}`; }
  getDetails(): string {
    return `${super.getLastName()}, ${super.getFirstName()} from ${super.getHomeCountry()} -- ${this.jobTitle}, ${this.department}`;
  }
  getSummary(): string { return `${super.getLastName()}, ${this.department}`; }
}

const newEmployee = new Employee('Mike', 'Johnson', 'Canada', 'Software Developer', 'Engineering');
console.log("We have a new employee in the department " + newEmployee);

// newEmployee.firstName = 'Dan';
...
const reportableArray : Reportable[] = [];
